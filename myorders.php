...
<div class="container-md">
	<h3><u>YOUR ORDERS -</u></h3> <br>
	 <?php 
	 $id= $_SESSION['uid'];
	 $q = mysqli_query($con,"select DISTINCT order_id,order_date,status,bill_amt,uname,email,uaddress,umobno from orders,user where orders.uid=user.uid and orders.uid=".$_SESSION['uid']);
	 while($r=mysqli_fetch_array($q)){
	 ?>
	 <table class="table border">

	<?php $oq = mysqli_query($con,"select * from orders,tblproduct,user where orders.pid=tblproduct.pid and orders.uid=user.uid and orders.uid='$id' and order_id=".$r['order_id']); ?>
...
<?php } ?>
<tr>
	<td><a class="btn btn-primary btn-rounded" href="printbill.php?order_id=<?=$r['order_id']?>">Download Invoice</a></td>
	<td></td>
